    ...
    switch (entry.type) {
      ...
      case 'business':
        if ('businessId' in entry) {
          ...
          // Get discount percentage if available (only if business accepts discounts)
          const acceptsDiscounts = fullBusiness?.businessInfo?.acceptsStandDiscounts === true;
          const discountPercent = acceptsDiscounts ? (fullBusiness?.businessInfo?.endorsementDiscountPercent || fullBusiness?.businessInfo?.customerDiscountPercent) : undefined;
